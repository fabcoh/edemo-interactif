...
import { InsertUser, users, presentationSessions, documents, presentationViewers, PresentationSession, Document, presentationFolders, PresentationFolder, presenterCursors, PresenterCursor } from "../drizzle/schema";
 ...
 */
export async function createPresentationFolder(
  userId: number,
  name: string,
  description?: string,
  color?: string
): Promise<PresentationFolder> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  await db.insert(presentationFolders).values({
    userId,
    name,
    description,
    color: color || "#3B82F6",
  ...
  const result = await db
    .select()
    .from(presentationFolders)
    .where(and(eq(presentationFolders.userId, userId), eq(presentationFolders.name, name)))
    .limit(1);

  if (result.length === 0) throw new Error("Failed to create folder");
  return result[0];
}
 ...
 */
export async function getUserFolders(userId: number): Promise<PresentationFolder[]> {
  const db = await getDb();
  if (!db) return [];
  ...
  return await db
    .select()
    .from(presentationFolders)
    .where(eq(presentationFolders.userId, userId));
}

/**
 * Delete a presentation folder
 */
export async function deletePresentationFolder(folderId: number): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  await db.delete(presentationFolders).where(eq(presentationFolders.id, folderId));
}

/**
 * Delete a presentation session
 */
export async function deletePresentationSession(sessionId: number): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  // Delete all viewers for the session
  await db.delete(presentationViewers).where(eq(presentationViewers.sessionId, sessionId));

  // Delete the session itself
  await db.delete(presentationSessions).where(eq(presentationSessions.id, sessionId));
}

// ============ PRESENTER CURSOR TRACKING ============

/**
 * Update presenter cursor position and zoom
 */
...
/**
 * Get presenter cursor position for a session
 */
...
export async function deleteUser(userId: number): Promise<void> {
  ...
  // Delete all sessions and documents for this user
  const userSessions = await db
    .select()
    .from(presentationSessions)
    .where(eq(presentationSessions.presenterId, userId));

  for (const session of userSessions) {
    await deletePresentationSession(session.id);
  }

  // Delete user folders
  await db.delete(presentationFolders).where(eq(presentationFolders.userId, userId));
